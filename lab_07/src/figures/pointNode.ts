...
class PointNodeBuilder {
  static build(no_del?: boolean): HTMLDivElement {
...
export default class PointNode {
  ...
  del_btn: HTMLInputElement | null;

  constructor(parent: HTMLDivElement, meta: any, remove_x?: boolean) {
    this.node = PointNodeBuilder.build(remove_x);
  ...
  private setupListeners() {
    if (this.del_btn !== null) this.setupDeleteListener();
  ...
  public validate() {
    let res_x = Number(this.x_i.value);
    if (Number.isNaN(res_x) || this.x_i.value === "")
      throw new Error(
        `Ошибка в значении координаты x поля ввода с данными ${this.data}`
      );
    let res_y = Number(this.y_i.value);
    if (Number.isNaN(res_y) || this.y_i.value === "")
      throw new Error(
