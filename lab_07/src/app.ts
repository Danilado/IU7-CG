...
export default class App {
  ...
  private setupFillListener() {
    ...
    document.querySelector("#cut")!.addEventListener("click", () => {
      try {
        let cutLines = getCutLines(this.ff.rectangle, this.ff.lines);
  ...
  private setupFiguresListeners() {
    ...
    this.ff.rectangle.node.addEventListener("figure_changed", (event) => {
      event.stopPropagation();
      this.update();
    });
  }
  ...
  private update() {
    ...
    try {
      this.graphics.drawRect(this.ff.rectangle, rectColorI.value);
    } catch (_err) {
      console.log(_err);
    }
    try {
